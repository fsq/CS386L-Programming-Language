...
Proof.
  intros v s.
  assert (H: beq_nat v v=true). {
      induction v as [|v' pre].
      + reflexivity.
      + simpl. rewrite pre. reflexivity.
    }
  destruct s as [|h t].
  - simpl.
    rewrite H.
...
Fixpoint beq_natlist (l1 l2 : natlist) : bool
  ...
  | h::t => match l2 with
           | nil => false
           | h1::t1 => match (beq_nat h1 h) with
                      | true => beq_natlist t t1
                      | false => false
                      end
            end
  end.
...
Proof.
  ...
  - reflexivity.
  - simpl. 
    assert (helper: beq_nat h h = true). {  
      induction h as [|h' Qpre].
      - reflexivity.
      - simpl. rewrite Qpre. reflexivity.
    }
    rewrite helper. 
    rewrite Hpre.
    reflexivity.
...
Theorem rev_inj_helper : forall (l1 l2:natlist),
   l1 = l2 -> rev l1 = rev l2.
Proof.
  intros l1 l2 H.
  rewrite H.
...
Proof.
  ...
  pose proof (rev_inj_helper (rev l1) (rev l2)) as H1.
  pose proof (H1 H).
...
There are zero elements in type baz. 
Because we can't actually create a value of type baz.
All the constructors takes at least one baz type value.
This becomes a recursive definition without a base case.
(* FILL IN HERE *)
*)
(** [] *)

(** $Date: 2017-11-16 00:44:48 -0600 (Thu, 16 Nov 2017) $ *)
