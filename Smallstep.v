...
Proof.
  ...
  generalize dependent y2.
  induction H1; intros y2 H2; inversion H2;subst; try solve_by_invert.
  - reflexivity.
  - apply IHstep in H4. rewrite H4. reflexivity.
  - inversion H3. rewrite<-H in H1. inversion H1.
  - inversion H. rewrite<-H0 in H5. inversion H5.
...
Proof.
  ...
  - apply multi_step with (P t1 y).
    apply ST_Plus2; repeat assumption. assumption.
Qed.
(** [] *)
...
Proof.
  ...
  - apply multi_refl.
  - eapply multi_trans with (y:=P (C n1) (C n2)).
    + eapply multi_trans with (y:=P (C n1) t2).
...
(* 
Induction on H: t\\n.
1. If t=C n. apply reflexive of multi.
2. If t=P t1 t2, t1\\n1, t2\\n2.
   From assumption we know t1==>*C n1, t2==>*C n2.
   
   P t1 t2==>*P (C n1) t2, by congruence.
   P (C n1) t2==>*P (C n1) (C n2), by congruence.
   P (C n1) (C n2)==>* C(n1 + n2), by multi_R.
   Thus from the transitivity of ==>* we know P t1 t2==>*C(n1+n2).
*)
*)
(** [] *)
...
Proof.
  ...
  induction Hs; intros.
  - inversion H. apply (E_Plus (C n1) (C n2));apply E_Const.
  - inversion H;subst. apply IHHs in H2. eapply E_Plus;assumption.
  - inversion H0;subst. apply IHHs in H5. eapply E_Plus;assumption.
Qed.
(** [] *)
...
Proof.
  ...
  unfold step_normal_form in H0. unfold normal_form in H0.
  assert (  value t' \/ (exists t0, t' ==> t0) ).
  apply strong_progress.
  destruct H1;induction H.
  - inversion H1. exists n. split. 
    + reflexivity.
    + apply E_Const.
  - apply IHmulti in H0; try assumption.
    destruct H0. destruct H0. exists x0.
    split.
    assumption. eapply step__eval. apply H. assumption.
  - unfold not in H0. destruct H1. 
...
Proof.
  ...
  - inversion H0;subst.
    ...
    + inversion H5;subst.
      ...
      { inversion H3;rewrite<-H2 in H;inversion H. }
      { inversion H3;rewrite<-H4 in H;inversion H. }
      { inversion H3;rewrite<-H1 in H;inversion H. }
      { inversion H3;rewrite<-H1 in H;inversion H. }
      { inversion H3;rewrite<-H2 in H;inversion H. }
  - inversion H1;subst.
...
Theorem not_strong:
  exists t,  normal_form step t /\ ~ value t.
Proof.
  unfold normal_form.
  exists (P (C 1) ttrue).
  split.
  - unfold not. intros. inversion H. inversion H0;subst. inversion H4. inversion H5.
...
Lemma unfold4 : forall {X:Type} (a b c d:list X), (a++b++c)++d=a++b++c++d.
Proof. 
  intros. rewrite<-app_assoc with (m:=b++c). rewrite<-app_assoc. reflexivity.
Qed.
...
Proof.
  intros st e.
  induction e;intros;
    try (eapply multi_step; constructor; constructor);
    try (simpl;rewrite unfold4).
...
(** $Date: 2017-11-13 14:16:27 -0600 (Mon, 13 Nov 2017) $ *)
