...
Rewrite can substitute part of the goal by another equation.
To use apply, the goal must exactly match the theorem being applied.
When we want to substitute the whole goal by another equation, 
and the the goal matches the applied theorem and rewrited thereom, 
we can do them both.
*)
(** [] *)
...
1. n=0. 
    If m=0, n=m.
    If m=S m, beq_nat 0 (S m') can't be true.
2. n=S n.
    If m=0, beq_nat (S n) 0 can't be true.
    If m=S m, beq_nat (S n) (S m) = true -> beq_nat n m = true.
    Apply IHn, we know forall m:nat, beq_nat n m = true -> n = m, thus n = m.
    So S n = S m.
    *)
(** [] *)
...
Proof.
  ...
  - destruct (f b) eqn : Fb.
    ...
    + rewrite Hb in Fb.
      rewrite Fb. destruct (f false) eqn: ff.
      { rewrite Fb. reflexivity. }
...
Proof.
  ...
  - intros m.
    destruct (beq_nat 0 m) eqn: H.
    + apply beq_nat_true in H. 
      rewrite H. rewrite<-beq_nat_refl. reflexivity.
    + destruct m.
      { inversion H. }
      { rewrite S_nbeq_0. reflexivity. }
  - intros m.
    destruct (beq_nat m (S n')) eqn: H.
    + apply beq_nat_true in H. 
      rewrite H. rewrite<-beq_nat_refl. reflexivity.
    + destruct m.
      { rewrite S_nbeq_0. reflexivity. }
      {  inversion H. apply Hpre. }
Qed.
(** [] *)
...
Proof.
  ...
  - intros l1 l2 H H1.
    ...
    + inversion H. destruct l2.
      { simpl in H1. inversion H1. }
      { simpl in H. inversion H. }
    + inversion H. destruct l2 eqn: dl2.
      { simpl in H. inversion H. }
      { simpl in H. inversion H.
        inversion H1.
        rewrite H5. 
...
Proof.
  intros X.
  induction l.
  - intros ls H. simpl in H. inversion H.
  - intros ls H.
    simpl in H.
    destruct (test x0) eqn: t0.
    + inversion H. rewrite<-H1. apply t0.
...
Fixpoint forallb {X:Type} (pred: X->bool) (l:list X): bool :=
  match l with
  | [] => true
  | h::t => if (pred h) then (forallb pred t) else false
  end.
...
Fixpoint existsb {X:Type} (pred: X->bool) (l:list X): bool :=
  match l with
  | [] => false
  | h::t => if (pred h) then true else (existsb pred t)
  end.
...
Proof.
  intros X pred l.
  destruct (existsb pred l) eqn:exl.
  - unfold existsb'. 
    induction l.
    + simpl. inversion exl.
    + simpl.
      destruct (pred x) eqn: predx.
      { simpl. reflexivity. }
      { simpl. inversion exl. rewrite predx in H0. 
        rewrite predx. rewrite H0. 
        destruct (existsb pred l) eqn: exs.
  ...
  - unfold existsb'.
    ...
    + simpl.
      destruct (pred x) eqn: predx.
      { simpl in exl. rewrite predx in exl. inversion exl. }
      { simpl in exl. rewrite predx in exl. apply IHl in exl. 
        simpl. apply exl. } 
Qed.

(** [] *)

(** $Date: 2018-01-13 15:44:48 -0600 (Sat, 13 Jan 2018) $ *)
