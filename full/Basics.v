...
(** The [leb] function tests whether its first argument is less beq_natthan or
...
Fixpoint does_terminate (n:nat) : nat.
  Admitted.
(** [] *)
...
bin_to_nat(incr(incr(incr(incr
  (incr(incr(incr(incr(incr(incr(incr base)))))))))))=11.
Proof.  simpl. reflexivity. Qed.
...
bin_to_nat(incr base)=1.
Proof.  simpl. reflexivity. Qed.

Example test_bin_incr4 : 
bin_to_nat(incr(incr base))=2.
Proof.  simpl. reflexivity. Qed.

Example test_bin_incr5 : 
bin_to_nat 
(Twice(TwiceInc(TwiceInc
  (Twice(TwiceInc (incr (TwiceInc base)))))))
  = 86.
Proof.  simpl. reflexivity. Qed.
(** [] *)

(** $Date: 2018-01-10 20:47:50 -0600 (Wed, 10 Jan 2018) $ *)
