...
(** Write an informal proof of [double_neg]:
   ...
   _Theorem_: [P] implies [~~P], for any proposition [P]. 
    Forall props P and H. 
    1. if P if false, theorem holds;
    2. if P is true.
        First of all we unfold neg sign.
        The theorem becomes (P->False)->False.
        Apply P to P->False we get False->False.
        By principle of explosion, the theorem holds.
    *)
(** [] *)
...
(* For all prop P, unfold the theorem, we get:
    P and (P->False) -> False.
    If P and (P->False), we apply P->False in P, and get a false.
    By principle of explosion, the theorem is true.
 *)
(** [] *)
...
Proof.
  ...
  intros [Ha Hb].
  split.
  - intros H. apply H1 in H. apply Ha in H. apply H.
  - intros H. apply Hb in H. apply H2 in H. apply H.
Qed.
(** [] *)
...
Proof.
  ...
  - induction l.
    ...
    + intros H. simpl in H. destruct H.
      ...
      { apply IHl in H. simpl. destruct H.
        exists x0. split.
        { destruct H. apply H. }
        { destruct H. right. apply H0. }}
  - intros [].
...
Proof.
  ...
  - intros H. 
    ...
    + simpl. simpl in H. 
      ...
      { apply IHl. intros x0 Q.
        apply H. right. apply Q. }
  - intros H. induction l.
    + intros x Q. inversion Q.
    + simpl. intros x0 Q. 
      destruct Q.
      { simpl in H. destruct H. rewrite<-H0. apply H. }
      { simpl in H. destruct H. apply IHl.
        { apply H1. }
        { apply H0. }}
Qed.
...
Lemma tr_rev_correct : forall X, @tr_rev X = @rev X.
  ...
  intros l.
  unfold tr_rev.
  rewrite tr_rev_helper.  apply app_nil_r.
Qed.
(** [] *)
...
Theorem evenb_double_conv_helper: forall n,
...
Theorem evenb_double_conv_helper1: forall n,
  evenb n=false -> evenb (S n)=true.
Proof.
  intros n.
  intros H.
  induction n.
...
Proof.
  ...
  - destruct (evenb n') eqn: Q.
    + apply  evenb_double_conv_helper in Q. 
      rewrite Q. destruct H.
      exists x. rewrite H. reflexivity.
    + apply evenb_double_conv_helper1 in Q.
      rewrite Q. destruct H.
...
Proof.
  ...
  - intros m H.
    destruct m.
    + inversion H. + unfold not. intros Q. inversion Q. 
  - intros m H.
    unfold not.
    destruct m.
    + intros Q. inversion Q.
    + intros Q. inversion Q. apply IHn in H. unfold not in H.
...
Proof.
  intros x y.
  split.
  - intros H. apply beq_nat_false in H. apply H.
  - intros H. unfold not in H. destruct (beq_nat x y) eqn:Q.
    + apply beq_nat_true in Q. apply H in Q. inversion Q.
    + reflexivity. 
Qed.
(** [] *)
...
Fixpoint beq_list {A : Type} (beq : A -> A -> bool)
  ...
  | a::l1' => match l2 with
           | nil => false
           | b::l2' => (beq a b) && (beq_list beq l1' l2')
            end
  end.



Lemma beq_list_true_iff_helper:
...
Proof.
  ...
  - induction l2.
    + reflexivity.
    + simpl. intros Q. inversion Q.
  - induction l2.
    + simpl. intros Q. inversion Q.
    + simpl. intros Q. apply andb_true_iff in Q.
      destruct Q. apply IHl1 in H1. apply H in H0. 
...
Proof.
  intros A beq.
  split. 
  - apply beq_list_true_iff_helper. apply H.
  - generalize dependent l2.
    ...
    + intros l2 Q.
      ...
      { inversion Q. }
      { simpl.
        assert(x=x0). { inversion Q. reflexivity. }
        assert(l1=l2). { inversion Q. reflexivity. }          
        apply IHl1 in H1. apply H in H0.
        rewrite H0. rewrite H1. reflexivity. }     
Qed.
(** [] *)
...
Proof.
  ...
  - intros Q.
    ...
    + simpl. 
      ...
      { assert(forallb test l = true). 
        { destruct (forallb test l) eqn:H.
          + reflexivity.
          + inversion Q. rewrite H in H1. 
            destruct (test x). - rewrite H. reflexivity. -rewrite H. reflexivity.
        }
        apply IHl in H.
        apply H. }
  - intros H.
    ...
    + reflexivity.
    + simpl. 
      simpl in H. destruct H. apply IHl in H0. rewrite H. rewrite H0. reflexivity.
Qed.
...
Proof.
  intro em. unfold excluded_middle in em. unfold not in em.
  intros X P H x.
  unfold not in H.
  destruct (em (P x)).
  - apply H0.
...
(** $Date: 2017-11-14 16:52:45 -0600 (Tue, 14 Nov 2017) $ *)
