...
Proof.
  ...
  - simpl. rewrite Hpre. rewrite plus_n_Sm. reflexivity.
Qed.
...
(** Briefly explain the difference between the tactics [destruct]
    and [induction].
DESTRUCT splits a value into several cases, determined by the programmer.

INDUCTION splits a value into two cases:
  1. base case 
  2. induction step: a n' along with a theorem which holds for n'.
*)
(** [] *)
...
(** Translate your solution for [plus_comm] into an informal proof:
    ...
    Proof: By induction on n.
        - First, suppose n=0. We must show
               0 + m = m + 0.
          Following from the definition of [+], 0 + m = m.
          Following from plus_n_O_secondtry we proved before, m + 0 = m.
          Immediately we have 0 + m = m + 0 = m.
  
        - Next, suppose [n = S n'], where
              n' + m = m + n'.
          We must show,
             S n' + m = m + S n'.
          By plus_n_Sm as we proved before, we have 
            S (n' + m) = S (m + n')
          Which is immediate from the induction hypothesis. _QED_.
...
Proof.
  ...
  - simpl. rewrite Hpre. rewrite mult_comm_helper. reflexivity.
Qed.
...
Theorem mult_plus_distr_r_helper : forall n m p q:nat,
  n + m + (p + q) = n + p + (m + q).
Proof.
  intros n m p q.
  induction n as [|n' Hpre].
  - simpl. rewrite plus_swap. reflexivity.
  - simpl. rewrite Hpre. reflexivity.
Qed.
...
Proof.
  ...
  - rewrite <- mult_comm_helper.
    ...
    rewrite <- mult_comm_helper.
    rewrite Hpre.
    rewrite mult_plus_distr_r_helper.
    reflexivity.
Qed.
...
Proof.
  ...
  induction n as [|n' Hpre].
  - simpl. reflexivity.
  - simpl. rewrite Hpre. rewrite mult_plus_distr_r. reflexivity.
Qed.
(** [] *)
...
Fixpoint bin_to_nat (b:bin) : nat :=
  ...
  | TwiceInc b' => 2 * (bin_to_nat b') + 1
  end.

Theorem nat_plus_1_eq_S_nat: forall x:bin, 
  bin_to_nat x + 1 = S (bin_to_nat x).
Proof.
  intros x.
  induction x as [|x1 H1|x2 H2].
  - simpl. reflexivity.
  - simpl. rewrite<-plus_n_O. 
    rewrite<-plus_assoc. rewrite H1.
    rewrite plus_n_Sm. reflexivity.
  - simpl. rewrite<-plus_n_O.
    rewrite plus_n_Sm. rewrite<-plus_assoc. simpl. reflexivity.
Qed.

Theorem plus_assoc_helper1 : forall n:bin,
  1 + (bin_to_nat n + bin_to_nat n + 1) =
  (bin_to_nat n+bin_to_nat n) +2.
Proof.
  intros n.
  induction n as [|n' H1|n' H2].
  - simpl. reflexivity.
  - simpl. rewrite<-plus_n_O. rewrite plus_n_Sm. reflexivity.
  - simpl. rewrite<-plus_n_O. rewrite plus_n_Sm. reflexivity.
Qed.

Theorem bin_helper3 : forall (n x:nat),
  n+n+x = x+n+n.
Proof.
  intros n x.
  rewrite plus_comm.
...
Theorem bin_helper2: forall (n x y:nat) ,
  n+n+x+y = x+y+n+n.
Proof.
  intros n x y.
  rewrite<-bin_helper3.
...
Theorem bin_helper4: forall (n x:nat),
  n+n+n+n+x = x+n+n+n+n.
Proof.
  intros n x.
  rewrite plus_comm.
  rewrite plus_assoc.
  replace (x+(n+n+n)) with (x+n+n+n). 
  - simpl. reflexivity.
  - rewrite plus_assoc.
    replace (x+n+n) with (x+(n+n)).
    + reflexivity.
...
Proof.
  ...
  - simpl. rewrite<-plus_n_O. rewrite<-plus_n_O.
    ...
    + simpl. rewrite<-plus_n_O. 
      ...
      rewrite bin_helper2.
      rewrite bin_helper2.
      rewrite plus_assoc_helper1.
      reflexivity.
    + simpl. rewrite<-plus_n_O.  rewrite<-plus_n_O.
      rewrite plus_n_Sm. rewrite H2.      
      rewrite plus_n_Sm. 
      rewrite bin_helper2.
      rewrite bin_helper2.    
      rewrite bin_helper3.
...
(** $Date: 2017-11-15 21:55:41 -0600 (Wed, 15 Nov 2017) $ *)
