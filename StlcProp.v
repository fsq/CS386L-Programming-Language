...
Proof.
  ...
  - inversion H1.
  - apply IHt2 in H4. apply IHt1 in H2.
    destruct H2.
    + destruct H4.
      * inversion Ht;subst. 
        ...
        eapply canonical_forms_fun; eauto.
        destruct H1 as [x0 [t0 Heq]];subst.
        right.  exists ([x0:=t2] t0). auto.
      * inversion Ht;subst. 
        destruct H0. right. exists (tapp t1 x0). constructor;assumption.
    + destruct H4;inversion Ht;subst;
      destruct H; right; exists(tapp x0 t2); constructor;assumption.
  - apply IHt1 in H3 as TT. destruct TT.
    + right. destruct (canonical_forms_bool t1); subst; eauto.
    + right. destruct H. exists (tif x0 t2 t3). constructor;auto.
Qed.
(** [] *)
...
(**
1. if a term is a single variable name x, then x appears free in it.
2. For application (tapp t1 t2), forall variable x appears free in 
   either t1 or t2, x appears free in application.
3. For function definition (tabs y T11 t12).a variable x appears free in it
   iff x appears free in t12 and x<>y
4. For a if statement, any variable x appears free among them also appears
    free in the whold term.
*)
(** [] *)
...
Corollary typable_empty__closed : forall t T,
    ...
    closed t.
Proof.
  intros. unfold closed,not;intros.
  remember empty as gamma.
  assert (exists T', gamma x0 = Some T'). 
...
(* 
False
(tapp (tabs x (TArrow TBool TBool) ttrue) ttrue)==>ttrue
t' has type bool
t is not bool 
 *)
*)
...
Proof.
  ...
  induction Hmulti.
  - apply progress in Hhas_type. destruct Hhas_type;eauto.
  - apply IHHmulti. 
    eapply preservation. apply Hhas_type. assumption. assumption. assumption.
Qed.
(** [] *)
...
Proof.
  ...
  induction H;intros T' He;inversion He;subst;try reflexivity.
  - rewrite H in H2. inversion H2. reflexivity.
  - apply IHhas_type in H5. rewrite H5. reflexivity.
  - apply IHhas_type1 in H4. inversion H4;subst. reflexivity.
  - apply IHhas_type2 in H8. apply H8. 
Qed.
(** [] *)
...
Theorem _progress : forall t T,
  empty |- t \in T ->
  value t \/ exists t', t ==> t'. Admitted.

Theorem _preservation: forall t t' T,
  empty |- t \in T  ->
  t ==> t'  ->
  empty |- t' \in T. Admitted.
...
and the following typing rule:
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
      becomes false.
      eg. (tif ttrue ttrue ttrue)==>ttrue /\(tif ttrue ttrue ttrue)=>zap
      - Progress
      becomes false.
      zag can't step
      - Preservation
      if t==>zap, use T_Zap, zap can have any type.
*)
(** [] *)
...
(** Suppose instead that we add a new term [foo] with the following 
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
      becomes false.
      (tapp (\x:A. x) y)==>y /\(tapp (\x:A. x) y)==>(tapp foo y)
      - Progress
      remains true
      - Preservation
      becomes false.
      foo can only be assessed to bool
*)
(** [] *)
...
(** Suppose instead that we remove the rule [ST_App1] from the [step]
    ...
    false, give a counterexample.
      ...
      remains true
      - Progress
      becomes false. some non-value terms cannot step
      e.g. (tapp (tif ttrue t t) ttrue)
      - Preservation
      remains true.
*)
(** [] *)
...
Proof with eauto.
  ...
  - (* T_App *)
    apply T_App with T11...
  - constructor.
  - constructor. apply IHhas_type. intros x He. apply H0. constructor. apply He.
  - constructor. apply IHhas_type. intros x He. apply H0. constructor. apply He.
  - constructor. 
    + apply IHhas_type1. intros x He. apply H2. auto. 
    + apply IHhas_type2. intros x He. apply H2. auto. 
    + apply IHhas_type3. intros x He. apply H2. auto. 
Qed.
...
Proof with eauto.
  ...
  - (* tvar *)
    ...
    + (* x<>y *)
      apply T_Var. rewrite update_neq in H2...
  - eapply T_App. apply IHt1. apply H3. apply IHt2. apply H5.
  - (* tabs *)
...
Proof with eauto.
  ...
  - (* T_App *)
    ...
    + (* ST_AppAbs *)
      apply substitution_preserves_typing with T11...
      inversion HT1...
    +  eapply T_App. apply IHHT1;auto. auto.
    +  eapply T_App;auto. auto.
  - inversion HE;subst;constructor;auto.
  - inversion HE;subst;constructor;auto.
  - inversion HE;subst;auto;constructor;auto. 
Qed.
...
Proof with eauto.
  ...
  - (* T_App *)
    ...
    + (* t1 is a value *)
      ...
      * (* t2 is also a value *)
        ...
        eapply canonical_forms_fun; eauto.
        destruct H1 as [x0 [t0 Heq]]. subst.
        exists ([x0:=t2]t0). constructor. auto.
  ...
  - assert ( value t \/ (exists t' : tm, t ==> t')). apply IHHt. reflexivity.
    destruct H.
    + apply canonical_forms_nat in Ht. destruct Ht. 
      right. rewrite H0. exists (tnat (S x0)). constructor. auto.
    + destruct H. right. exists (tsucc x0). constructor. apply H.
  -  assert ( value t \/ (exists t' : tm, t ==> t')). apply IHHt. reflexivity.
    destruct H.
    + apply canonical_forms_nat in Ht. destruct Ht. 
      right. rewrite H0. destruct x0.
      exists (tnat 0). constructor. 
      exists (tnat x0). constructor. auto.
    + destruct H. right. exists (tpred x0). constructor. apply H.
  - (* T_If *)
    right. destruct IHHt1...

    + (* t1 is a value *)
      apply canonical_forms_nat in H. destruct H.
      destruct x0;subst.
      * exists t2. constructor.
      * exists t3. constructor.
      * auto.
    + (* t1 also steps *)
      inversion H as [t1' Hstp]. exists (tif0 t1' t2 t3);constructor;auto.
Qed.
(** [] *)

End STLCArith.

(** $Date: 2017-11-18 15:56:46 -0600 (Sat, 18 Nov 2017) $ *)
