...
Proof.
  intros n m.
  induction n.
  - intros H Q. simpl. apply Q.
  - intros H Q. induction H.
    + simpl. apply Q.
...
Proof.
  ...
  induction Q.
  + simpl in H. apply H.
  + simpl in H. inversion H. apply IHQ in H1. apply H1.
Qed.
(** [] *)
...
Proof.
  ...
  induction H.
  - apply le_S. apply le_n.
  - apply le_S. apply (le_trans (S n) m (S m)).
    + apply H.
    + apply le_S. apply le_n.
Qed.
...
- the first is provable. apply c2. apply c3. apply c1.
  the second is unprovable.

- no. proof of the first proposition doesn't need c5.
...
Inductive subseq: list nat->list nat->Prop :=
  | ss_0 l: subseq [] l
  | ss_2 x l1 l2: subseq l1 l2->subseq (x::l1) (x::l2)
  | ss_1 x l1 l2: subseq l1 l2->subseq l1 (x::l2).
...
Proof.
  ...
  induction H.
  - apply ss_0.
  - simpl. apply ss_2. apply IHsubseq.
  - simpl. apply ss_1. apply IHsubseq. 
Qed.
...
Proof.
  ...
  - simpl. apply MStarApp.
    ...
    + apply IHss. intros s Q.
      apply H. simpl. right. apply Q.
Qed.
(** [] *)
...
Proof.
  ...
  - split.
    ...
    + inversion H. intros Z. apply H0.
  - split.
    + inversion H. intros Z. unfold not in H0. apply H0 in Z. destruct Z.
    + inversion H. intros Z. inversion Z.
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. intros H. unfold not. intros Q. inversion Q.
  - simpl. destruct (beq_natP n x).
    + intros Q. unfold not. intros P. 
      destruct P.
      { inversion Q. }
      { inversion Q. }
    + intros Q. unfold not. intros P.
      destruct P.
      { simpl in Q. rewrite H0 in H. destruct H. reflexivity. }
      { simpl in Q. apply IHl in Q. unfold not in Q. apply Q in H0. apply H0. }
Qed.
(** [] *)
...
Inductive nostutter {X:Type} : list X -> Prop :=
  | ns0 : nostutter []
  | ns1 : forall x: X, nostutter [x]
  | ns2 : forall (x y: X) (l:list X), 
...
Inductive is_merge {X}: list X->list X->list X->Prop :=
  | z  : is_merge [] [] []
  | ml : forall (l l1 l2:list X) (x:X), 
          is_merge l l1 l2->is_merge (x::l) (x::l1) l2
  | mr : forall (l l1 l2:list X) (x:X),
          is_merge l l1 l2->is_merge (x::l) l1 (x::l2).


Theorem negb_true_iff: forall b:bool,
  negb b = true-> b=false.
Proof.
  intros.
  destruct b.
...
Theorem filter_challenge1: 
  forall {X} (l l1 l2:list X) (test:X->bool),
  is_merge l l1 l2 
    -> All (fun a=>test a=true) l1
    -> All (fun a=> negb (test a)=true) l2
    -> filter test l = l1.
Proof.
  intros.
  induction H.
  - reflexivity.
  - simpl. destruct (test x) eqn:TX.
    + apply forallb_true_iff in H0. simpl in H0. 
      apply andb_true_iff in H0. destruct H0.
      apply forallb_true_iff in H2. apply IHis_merge in H2. 
      { rewrite H2. reflexivity. }
      { apply H1. }
    + apply forallb_true_iff in H0. simpl in H0. 
      apply andb_true_iff in H0. destruct H0. rewrite H0 in TX. inversion TX.
  - apply forallb_true_iff in H1. simpl in H1. 
    ...
    + apply  IHis_merge.
      { apply H0. }
      { apply IHis_merge in H0. 
        { apply forallb_true_iff in H2. apply H2. }
        { apply forallb_true_iff in H2. apply H2. }}
...
(** $Date: 2017-11-15 13:12:04 -0600 (Wed, 15 Nov 2017) $ *)
