...
Proof.
  ...
  - eapply hoare_consequence_post.
    apply hoare_while.
    + intros st H Q [W1 W2]. inversion Q;subst.  simpl.
      assert (st & {X-->st X-2} X = st X-2).
      apply t_update_eq.
      rewrite H. rewrite<-W1. apply parity_ge_2.
      unfold bassn in W2. unfold beval in W2. 
      apply leb_complete in W2. simpl in W2. apply W2.
    + intros st H. destruct H. 
      unfold bassn in H0. apply not_true_iff_false in H0. 
      unfold beval in H0. apply leb_iff_conv in H0. simpl in H0.
      assert (~2<=st X). unfold not. intros fls. omega.
      apply parity_lt_2 in H1. rewrite<-H1. apply H.
...
Example factorial_dec (m:nat) : decorated :=
  ...
  WHILE !(X = 0)
  DO   {{fun st=>(real_fact (st X)) *st  Y = real_fact m  /\ (st X)<>0 }} ->>
...
Theorem fact_next:
  forall x, x<>0->real_fact (x-1) * x = real_fact x.
Proof.
  induction x.
  - intros H. exfalso. apply not_eq in H. destruct H;inversion H.
  - intros H. simpl. rewrite<-minus_n_O. rewrite<-mult_n_Sm. 
...
Proof.
  ...
  - rewrite (mult_comm (st Y) (st X)). 
    rewrite (mult_assoc (real_fact (st X - 1)) (st X)).
    rewrite fact_next;assumption.
  - simpl in H. rewrite plus_comm in H. rewrite plus_O_n in H. apply H.
Qed.
...
(** $Date: 2017-11-21 12:26:10 -0600 (Tue, 21 Nov 2017) $ *)
