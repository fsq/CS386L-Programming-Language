...
(* 
as1: the value of X is 3
as2: the value of X ls less or equal than the value of Y
as3: as1 is true or as2 is true
as4: z^2<=x and(z+1)*(z+1)>x
as5: always true
as6: always false
 *)
End ExAssertions.
...
(* let a=X+1, then X::=a means increase X by 1. 
   But the later assertion {{X=a}} -> {{X==X+1}} will never be true for nats.
 *)
(** [] *)
...
Proof.
  ...
  assert ((st & {X --> aeval st a; X --> m})=st & {X-->m}).
  apply t_update_shadow.
  assert ((st & {X-->m})=st).
  rewrite<-H1.  apply t_update_same. 
  split.
  - rewrite H2. 
    rewrite H3. apply H0.
  - assert (((st & {X --> aeval st a}) X)=aeval st a).
    apply t_update_eq. rewrite H4. 
...
(*
  If a=X;
  after the com, X=3,Y=a=X=3. n is lost.
 *)
(** [] *)
...
Proof.
  ...
  - eapply hoare_consequence_pre. apply hoare_asgn.
    unfold bassn, assn_sub, t_update, assert_implies. 
    simpl; intros. destruct H. apply leb_complete in H0. omega.
  - eapply hoare_consequence_pre. apply hoare_asgn.
    unfold bassn, assn_sub, t_update, assert_implies.
    simpl; intros. destruct H. 
    apply not_true_is_false in H0. apply leb_complete_conv in H0.
    omega.
...
Inductive ceval : com -> state -> state -> Prop :=
  ...
  | E_WhileTrue : forall (st st' st'' : state) (b1 : bexp) (c1 : com),
                  ...
                  (WHILE b1 DO c1 END) / st \\ st''
  | E_If1True: forall (st st':state) (b:bexp) (c1:com),
                  beval st b = true->
                  c1 / st \\ st'-> (IF1 b THEN c1 FI) /st \\ st'
  | E_If1False: forall (st :state) (b:bexp) (c1:com),
                  beval st b = false->
                  (IF1 b THEN c1 FI) /st \\ st
...
Proof.
  intros. unfold hoare_triple;intros. 
  inversion H1; subst.
  - eapply H. apply H8. split. apply H2. apply bexp_eval_true. apply H5.
  - eapply H0. apply E_Skip. split. apply H2. 
    unfold not. intros Z. apply bexp_eval_false in H7. unfold not in H7. 
    apply H7 in Z. inversion Z.
...
Example hoare_if1_good:
  ...
  {{fun st=> st X = st Z }}.
Proof.
  apply hoare_if1; unfold hoare_triple;intros.
  - destruct H0. inversion H;subst. 
    rewrite t_update_eq. simpl. rewrite t_update_neq.
    apply H0. destruct (beq_string X Z) eqn:T.
    + inversion T. 
    + apply beq_string_false_iff in T. apply T.
  - destruct H0. inversion H;subst. 
    unfold bassn in H1. apply not_true_is_false in H1. simpl in H1.
    apply negb_false_iff in H1. apply beq_nat_true in H1.
    rewrite H1 in H0. rewrite (plus_comm (st' X) 0) in H0.
    rewrite plus_0_l in H0. apply H0.
Qed.
...
Inductive ceval : state -> com -> state -> Prop :=
  ...
  | E_WhileTrue : forall st st' st'' b1 c1,
      ...
      ceval st' (WHILE b1 DO c1 END) st'' ->
      ceval st (WHILE b1 DO c1 END) st''
  | E_NoRepeat: forall st st' b1 e1,
      ceval st e1 st'->
      beval st' b1 = true ->
      ceval st (REPEAT e1 UNTIL b1 END) st'
  | E_Repeat: forall e1 st st' st'' b1,
      ceval st e1 st' ->
      beval st' b1 = false ->
      ceval st' (REPEAT e1 UNTIL b1 END) st''->
      ceval st  (REPEAT e1 UNTIL b1 END) st''
...
Proof.
  ...
  remember (REPEAT c UNTIL b END) as rcom eqn:Req.
  induction H1;inversion Req;subst.
  - intros. split. eapply H2. apply H1. apply H3. apply bexp_eval_true. apply H0.
  - intros. eapply IHceval2. reflexivity.
    apply H. simpl. split. 
    eapply H1. apply H1_. apply H2.  apply bexp_eval_false. apply H0. 
Qed.
...
(** $Date: 2017-12-05 10:28:10 -0600 (Tue, 05 Dec 2017) $ *)
