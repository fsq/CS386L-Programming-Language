...
Proof.
  ...
  inversion H.
  + inversion H0. inversion H1;subst;inversion H2.
  + induction H1.
    - destruct H0. inversion H0.
    - apply IHnvalue. inversion H1;subst.
      right. assumption. right. assumption. destruct H0.
      inversion H0;subst. exists t1'. assumption.
...
Proof with auto.
  ...
  - inversion HT;subst.
    + destruct IHHT.
      * left. right. apply nv_succ. apply nat_canonical in H2; assumption.
      * right. destruct H2. exists (tsucc x). apply ST_Succ. assumption.
    + left. right. constructor. constructor.
    + destruct IHHT.
      * left. right. constructor. apply nat_canonical in HT;assumption.
      * right. destruct H0. exists (tsucc x). constructor. assumption.
    + destruct IHHT.
      * left. right. constructor. apply nat_canonical in HT;assumption.
      * right. destruct H0. exists (tsucc x). constructor. assumption.
  - right. inversion IHHT; clear IHHT;inversion H.
    solve_by_inverts 2. 
    + inversion H0. exists tzero. constructor. exists t. constructor. assumption.
    + exists (tpred x). constructor. apply H0.
  - right;inversion IHHT;clear IHHT.
    + inversion H. solve_by_inverts 2. inversion H0.
      exists (ttrue). constructor. exists (tfalse). constructor. apply H1.
    + destruct H. exists (tiszero x). constructor. apply H.
Qed.
(** [] *)
...
Proof with eauto.
  ...
  generalize dependent T.
  induction H0;intros;
  try (inversion H;subst;constructor;try assumption);
  try (inversion H;subst;assumption);
  try (apply IHstep;assumption).
...
(** Having seen the subject reduction property, one might
   ...
   t=(tif ttrue t' tzero) ==> t'
   but (tif ttrue t' tzero) is neither NAT nor BOOL
...
(** $Date: 2017-11-21 11:43:02 -0600 (Tue, 21 Nov 2017) $ *)
