...
Proof.
  ...
  - inversion Hb; subst.
    + apply E_IfFalse.
      { unfold beval. unfold beval in H4. rewrite H4. reflexivity. }
      { apply H5. }
    + apply E_IfTrue.
      { unfold beval. unfold beval in H4. rewrite H4. reflexivity. }
      { apply H5. }
  - inversion Hb; subst.
    + apply E_IfFalse.
      { inversion H4. apply negb_true_iff in H0. apply H0. }
      { apply H5. }
    + apply E_IfTrue.
      { inversion H4. apply negb_false_iff in H0. apply H0. }
      { apply H5. }
Qed.
(** [] *)
...
Proof.
  ...
  inversion H0. simpl. unfold bequiv. intros. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  split;intros Hb.
  - unfold aequiv in H.
    replace st' with (st' & {X --> aeval st' X});inversion Hb;subst. 
    + apply E_Ass with (a1 := e). symmetry. apply H.
    + inversion Hb. subst. apply t_update_same.
  - unfold aequiv in H.
    inversion Hb; subst.
    rewrite<-H.
...
Definition equiv_classes : list (list com) :=
  [[prog_f;prog_g];[prog_e;prog_b];[prog_c;prog_h];[prog_a];[prog_d];[prog_i]].
(** [] *)
...
Proof.
  ...
  - inversion He;subst.
    + apply E_IfTrue. 
      { unfold bequiv in H. rewrite H in H7. apply H7. }
      { unfold cequiv in H0. rewrite H0 in H8. apply H8. }
    + apply E_IfFalse.
      { unfold bequiv in H. rewrite H in H7. apply H7. }
      { unfold cequiv in H1. rewrite H1 in H8. apply H8. }
  - inversion He;subst.
    + apply E_IfTrue. 
      { unfold bequiv in H. rewrite<-H in H7. apply H7. }
      { unfold cequiv in H0. rewrite<-H0 in H8. apply H8. }
...
Proof.
  ...
  -  remember (WHILE b DO c END) as cond.
     assert(bequiv b (fold_constants_bexp b)). 
     apply fold_constants_bexp_sound.
     destruct (fold_constants_bexp b) eqn:He;
     try (rewrite Heqcond; apply CWhile_congruence; assumption; assumption).
     + apply (WHILE_true b c )in H . simpl in H. rewrite Heqcond. apply H.
     + apply (WHILE_false b c) in H. rewrite Heqcond. apply H.
Qed.
(** [] *)
...
Proof.
  ...
  remember true as b. remember SKIP as c.
  assert (bequiv b BTrue). rewrite Heqb. simpl. apply refl_bequiv.
  assert (forall st st', ~(WHILE b DO c END) /st \\st').
  { intros st st'. apply (WHILE_true_nonterm b c ). apply H. }
  assert (forall st, (WHILE b DO c END) /st \\ st).
  { intros st. apply Contra. rewrite Heqc. apply E_Skip. }
  assert (forall st, (~(WHILE b DO c END) /st \\ st)).
  { intros st. apply H0. }
  subst.
  assert (~exists (st:state), (WHILE true DO SKIP END) /st \\st).
  { unfold not. intros Heq. destruct Heq.
    assert ( ~(WHILE true DO SKIP END) /x \\x). apply H2.
    unfold not in H4. apply H4 in H3. inversion H3. }
  assert (exists (st:state), (WHILE true DO SKIP END) /st \\st).
  { exists { --> 0 }. apply H1. }
...
Inductive ceval : com -> state -> state -> Prop :=
  ...
  | E_WhileTrue : forall (st st' st'' : state) (b1 : bexp) (c1 : com),
      ...
      (WHILE b1 DO c1 END) / st' \\ st'' ->
      (WHILE b1 DO c1 END) / st \\ st''
  | E_Havoc: forall (n:nat) (X:string) (st st':state) ,
      (X::=n) /st \\ st' -> (HAVOC X) / st \\ st'
...
Proof. apply (E_Havoc 0). apply E_Ass. reflexivity. Qed.

Example havoc_example2 :
  (SKIP;; HAVOC Z) / { --> 0 } \\ { Z --> 42 }.
Proof.
  apply (E_Seq SKIP (HAVOC Z) {-->0} {-->0} {Z-->42}).
  apply E_Skip.
  apply (E_Havoc 42). constructor. reflexivity.
Qed.
(** [] *)
...
Lemma peq :
  cequiv pXY pYX.
Proof.
  unfold pXY; unfold pYX; unfold cequiv.
  intros; split;intros H.
  - destruct (beq_string X Y) eqn:eq.
    + apply beq_string_true_iff in eq;
    ...
    + try (inversion H;
            ...
            subst; simpl).
     assert (forall (s:state), s& {X --> n; Y --> n0} = s&{Y --> n0; X --> n})
        by (intros; apply t_update_permute; apply beq_string_false_iff; apply eq).
      apply E_Seq with (st & {Y-->n0}). 
      apply E_Havoc with (n0).
  ...
  - destruct (beq_string X Y) eqn:eq.
    ...
    + try (inversion H;
            ...
            inversion H7; inversion H11;
            subst; simpl).
     assert (forall (s:state), s& {X --> n0; Y --> n} = s&{Y --> n; X --> n0})
        by (intros; apply t_update_permute; apply beq_string_false_iff; apply eq).
      apply E_Seq with (st & {X-->n0}). 
      apply E_Havoc with (n0).
      constructor. simpl. reflexivity.
      apply E_Havoc with (n).
      rewrite<-H0.
...
Lemma WHILE_true_nonterm1 : forall b (c:com) st st',
  bequiv b BTrue ->
  ~( (WHILE b DO c END) / st \\ st' ).
Proof.
  (* WORKED IN CLASS *)
...
Proof. 
  unfold not,p1.
  intros.
  remember ((WHILE ! (X = 0) DO HAVOC Y;; X ::= X + 1 END)) as c.
  induction H0; inversion Heqc.
  - rewrite H2 in H0. inversion H0. apply negb_false_iff in H4.
  apply beq_nat_true in H4. apply H in H4. inversion H4.
  - apply IHceval2. rewrite H3 in H0_.
    inversion H0_;subst;
    inversion H5;inversion H8;subst;simpl.
    + intros. rewrite (t_update_eq nat st'0 X ) in H1. omega.
...
Proof.
  unfold p2, not.
  intros.
  remember (WHILE ! (X = 0) DO SKIP END) as c.
...
Proof. 
  ...
  - inversion H; subst; simpl; unfold p2.
    + apply E_WhileFalse. simpl. rewrite eq. reflexivity.
    + inversion H2; subst; simpl. rewrite eq in H1. inversion H1.
...
Proof. 
  unfold not, cequiv.
  intros.
  remember ({X --> 2; Z --> 1; X --> 0;Z --> 123}) as final.
  assert(G :  p3 /{X-->2} \\ final
          <-> p4 /{X-->2} \\ final). apply H. 
  inversion G. 
  assert(G2 : p3 / {X-->2}\\final).
  { apply E_Seq with ({X-->2;Z-->1}).
     - constructor; reflexivity.
     -  rewrite Heqfinal. apply E_WhileTrue with (final). 
        + reflexivity.
        + apply E_Seq with ({X --> 2; Z --> 1; X --> 0}).
          { apply E_Havoc with 0. constructor. reflexivity. }
          { apply E_Havoc with 123. rewrite Heqfinal. constructor. reflexivity. }
        +  rewrite <- Heqfinal. apply E_WhileFalse. rewrite Heqfinal. reflexivity. }
  apply H0 in G2.
...
(** $Date: 2017-10-25 15:33:13 -0500 (Wed, 25 Oct 2017) $ *)
